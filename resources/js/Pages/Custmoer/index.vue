...
const deleteItem = id => {
  Inertia.delete(route('customer.destroy', { item: id }), {
    onBefore: () => confirm('本当に削除しますか？')
  })
}

const searchText = () => {
  Inertia.get(route('custmoer.index', {search: search.value}))
}
                        ...
                        <div class="container px-5 py-8 mx-auto">
                          <FlashMessage />
                            <div class="flex pl-4 my-4 lg:w-2/3 w-full mx-auto">
                                <Link as="button" :href="route('custmoer.create')" class="flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">
                                    顧客登録
                                </Link>
                            </div>
                            <div>
                                    ...
                                    <tbody>
                                        <tr v-for="customer in customers.data" :key="customer.id">
                                            <td class="px-4 py-3 border-b-2 border-gray-200">
                                              <Link class="text-blue-400" :href="route('custmoer.show', {custmoer: customer.id})">
                                                {{ customer.id }}
                                              </Link>
                                            </td>
